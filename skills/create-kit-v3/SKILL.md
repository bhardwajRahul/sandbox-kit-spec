...
| `agent-context@1` | instructions the agent reads | `filename:` is for the kit that owns the environment: a workload, or a set that **resolves to** one. A set of only mixins derives `kind: mixin`, and validation then rejects its `filename` — the authoring-time check exempts `kind: set` because the kind is not known yet, so this one surfaces at publish. Use `contentFile:` for a static body, but inline `content:` when the body interpolates an arg — a staged body is never arg-expanded. |
| `long-running@1` | headless workloads that outlive client sessions | Workload-only, config-less. A background hook or published port does not prevent session auto-stop. Required by default; use `optional: true` only if auto-stop is tolerable. This does not request restart after failure. |
